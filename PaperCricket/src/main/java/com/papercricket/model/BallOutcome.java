...
public class BallOutcome {
	public static void playBall(Player player1, Player player2, Room room) {
		...
		if (room.getCurrentInnings() == 1 && (batting.getWickets() == 5 || room.getBallsElapsed() == 30)) {
			inningsChange(batting, bowling, room);
		} else if (room.getCurrentInnings() == 2 && (batting.getWickets() == 5 || room.getBallsElapsed() == 30 || batting.getScore() >= room.getTarget())) {
